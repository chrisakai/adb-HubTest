...
if __name__ == "__main__":
    ...
    while True:
        ...
        if not devices_init:
            ...
            logging.info(f"1查所有设备在线状态,成功, {devices_init}", extra={'count': count, 'deviceID': "新轮次", 'result': "新轮次"})
            ...
            for key in hub_map.keys():
                ...
                if hub_map.get(key) in devices_init.keys():
                    ...
                    try:
                        ...
                        response8 = requests.post(closeDoorURL, data=json_array,
                        ...
                        if response8.status_code == 200:
                            ...
                            logging.error(f"8关闭HUB对应端口{key},失败,{response8.status_code}",
                                          extra={'count': count, 'deviceID': device_id, 'result': status})
                            continue
                    except Exception as e:
                    ...
                    try:
                        ...
                        if not devices_after:
                            ...
                            logging.info(f"10查所有设备在线状态,成功,{devices_after}",
                                         extra={'count': count, 'deviceID': device_id, 'result': status})
                            compare_devices_differences(devices_init, devices_after, devices_map_log, count,
                                                        "10查所有设备在线状态",
                                                        device_id)
